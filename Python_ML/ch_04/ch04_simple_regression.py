...
# 선형 회귀식 계수
print('>>선형 회귀 모델 : ', end='')
print('y = {0:4.2f}x + {1:4.2f}'.format(lr.coef_[0], lr.intercept_))

# 알고리즘 평가
print('>>훈련 스코어 =', format(lr.score(train_input, train_target), '2.2f'))
print('>>테스트 스코어 =', format(lr.score(test_input, test_target), '2.2f'))

# 훈련 결과 평가
test_prediction = lr.predict(test_input)                        # 테스트 세트 예측값
...
plt.plot(point, lr.coef_[0]*point + lr.intercept_, label="Predictions", color='black')  # 예측 모델

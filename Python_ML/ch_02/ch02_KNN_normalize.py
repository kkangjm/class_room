...
# 데이터 Z-score 전처리 : (x - mean) / std.dev
...
# 데이터 플로팅
plt.scatter(normalized_data[:35, 0], normalized_data[:35, 1], label='tomato', color='red')
plt.scatter(normalized_data[36:, 0], normalized_data[36:, 1], label='mini tomato', color='blue')
plt.scatter(new_data[0], new_data[1], label='new data', marker='^')
plt.scatter(normalized_data[indexes, 0], normalized_data[indexes, 1], label='neighbors', marker='D')
plt.xlabel('size')
plt.ylabel('weight')
plt.legend()
plt.show()
